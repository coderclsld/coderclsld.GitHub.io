...
3. 为什么G1向缓存满了的channel中发送数据时被阻塞，在G2后来接收时，不将阻塞的G1发送的数据直接拷贝到G2中呢？

这是因为channel中的数据是队列的，要保证顺序，当有消费者G2接收数据时，需要先接收缓存中的数据。



4. 多个goroutine向有缓存的channel接收发送数据时，是可以保证顺序的吗？
